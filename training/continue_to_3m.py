"""
Continue training PPO model from 1.5M to 3M timesteps

This script loads the 1.5M checkpoint and continues training
for an additional 1.5M timesteps to reach 3M total.
"""
import os
import sys
import argparse
...
def main():
    parser = argparse.ArgumentParser(description='Continue PPO training from 1.5M to 3M timesteps')
    parser.add_argument('--checkpoint', type=str,
                        default=os.path.join(PROJECT_ROOT, 'output', 'ppo_1_5m_output', 'checkpoint_1503232.pt'),
                        help='Path to starting checkpoint')
    parser.add_argument('--output-dir', type=str,
                        default=os.path.join(PROJECT_ROOT, 'output', 'ppo_3m_output'),
                        help='Output directory for checkpoints')
    parser.add_argument('--additional-timesteps', type=int, default=1500000,
    ...
    print('=' * 80)
    print('CONTINUING PPO TRAINING: 1.5M -> 3M TIMESTEPS')
    print('=' * 80)
    print()
    print(f'Checkpoint: {checkpoint_path}')
    print(f'Additional timesteps: {additional_timesteps:,}')
    print(f'Target total: 3,000,000 timesteps')
    print()
    ...
    previous_timesteps = checkpoint.get('total_timesteps', 1503232)

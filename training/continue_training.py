...
def continue_training(
    ...
    # Final evaluation
    print("\nFinal evaluation:")
    eval_stats = evaluate(env, policy, n_episodes=50, device=device)
    print(f"Success Rate: {eval_stats['success_rate']*100:.1f}% ({eval_stats['successes']}/50)")
    print(f"Mean Reward: {eval_stats['mean_reward']:.2f}")


def evaluate(env, policy, n_episodes=50, device='cpu'):
    """Evaluate policy success rate and mean reward."""
    successes = 0
    total_reward = 0.0
